...
const handleSpinClick = async () => {
  ...
  try {
    // The winning number will now be generated by the backend
    // We'll get it from the response
    ...
    let winningNumberFromBackend = null;
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled' && result.value?.resolvedBet && result.value.winningNumber) {
        ...
        if (!winningNumberFound) {
          winningNumberFromBackend = result.value.winningNumber;
          setSpinResultNumber(String(winningNumberFromBackend));
    ...
    // Use the winning number from the backend
    const displayWinningNumber = winningNumberFromBackend !== null ? winningNumberFromBackend : 0;

    setMessage({
      text: `Spin result: ${displayWinningNumber}. ${profit === 0 ? 'No change.' : `You ${profit > 0 ? 'won' : 'lost'} $${Math.abs(profit).toFixed(2)}.`}`,
      type: profit > 0 ? 'success' : profit < 0 ? 'danger' : 'info',
    });

    setBetHistory((prev) => [displayWinningNumber, ...prev.slice(0, 9)]);
    fetchUserGameHistory();
    ...
    setSpinResultNumber(displayWinningNumber);
    setStartSpin(true);
