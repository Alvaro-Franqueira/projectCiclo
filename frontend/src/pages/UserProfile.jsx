...
import { Container, Row, Col, Card, Table, Badge, Alert, Tabs, Tab, Button, Spinner } from 'react-bootstrap';
...
const UserProfile = () => {
    ...
    const fetchProfileData = useCallback(async (userId) => {
        // ... (fetchProfileData implementation - no changes needed here) ...
        if (!userId) return;
    ...
    const fetchGameSpecificStats = async (userId) => {
        // ... (fetchGameSpecificStats implementation - no changes needed here) ...
        if (!userId) return;
        try {
    ...
    useEffect(() => {
        // ... (useEffect implementation - no changes needed here) ...
    ...
    const handleRefresh = () => {
        // ... (handleRefresh implementation - no changes needed here) ...
    ...
    const calculateStats = () => {
        // ... (calculateStats implementation - no changes needed here) ...
        ...
        <Container>
            ...
            <Row>
                ...
                <Col lg={4} className="mb-4 ">
                    {/* ... (Profile Card content - no changes needed here) ... */}
                        ...
                        <Card.Header>
                            <Tabs defaultActiveKey="stats" className="card-header-tabs">
                                ...
                                <Tab eventKey="stats" title={<><FaCoins className="me-1" /> Statistics</>}>
                                    {/* ... (Statistics Tab content - no changes needed here) ... */}
                                     <div className="p-3">
                                        <Row>
                                        ...
                                        <Row>
                                            <Col md={6} className="mb-3"> <Card className="h-100" style={{ backgroundColor: '#294c85' }}> <Card.Header className="d-flex justify-content-between align-items-center border-top-2" style={{ backgroundColor: '#011b45' }}> <span>Dice Game</span> <GiRollingDices size={20} color="#3498db" /> </Card.Header> <Card.Body> <div className="d-flex justify-content-between mb-2"> <span>Total Bets:</span> <span>{gameStats.dice.totalBets}</span> </div> <div className="d-flex justify-content-between mb-2"> <span>Win Rate:</span> <span className="text-info"> <FaPercentage className="me-1" />{(gameStats.dice.winRate ?? 0).toFixed(1)}% </span> </div> <div className="d-flex justify-content-between mb-2"> <span>Total Wins:</span> <span className="text-warning"> <FaTrophy className="me-1" />{gameStats.dice.totalWins} </span> </div> <div className="d-flex justify-content-between"> <span>Net Profit:</span> <span className={gameStats.dice.totalProfit >= 0 ? 'text-success' : 'text-danger'}> <FaCoins className="me-1" />${(gameStats.dice.totalProfit ?? 0).toFixed(2)} </span> </div> </Card.Body> </Card> </Col>
                                            <Col md={6} className="mb-3"> <Card className="h-100" style={{ backgroundColor: '#294c85' }}> <Card.Header className="d-flex justify-content-between align-items-center" style={{ backgroundColor: '#011b45' }}> <span>Roulette</span> <img src={rouletteImg} alt="Roulette Icon" width={40} height={30} /> </Card.Header> <Card.Body> <div className="d-flex justify-content-between mb-2"> <span>Total Bets:</span> <span>{gameStats.roulette.totalBets}</span> </div> <div className="d-flex justify-content-between mb-2"> <span>Win Rate:</span> <span className="text-info"> <FaPercentage className="me-1" />{(gameStats.roulette.winRate ?? 0).toFixed(1)}% </span> </div> <div className="d-flex justify-content-between mb-2"> <span>Total Wins:</span> <span className="text-warning"> <FaTrophy className="me-1" />{gameStats.roulette.totalWins} </span> </div> <div className="d-flex justify-content-between"> <span>Net Profit:</span> <span className={gameStats.roulette.totalProfit >= 0 ? 'text-success' : 'text-danger'}> <FaCoins className="me-1" />${(gameStats.roulette.totalProfit ?? 0).toFixed(2)} </span> </div> </Card.Body> </Card> </Col>
                                        </Row>
                                    </div>
                                </Tab>

                                {/* --- Bet History Tab (WHITESPACE FIXED) --- */}
                                ...
                                {/* --- My Rankings Tab (WHITESPACE FIXED) --- */}
                                            ...
                                            <Table responsive hover variant='dark'>
                                                ...
                                                <tbody>
                                                    {rankings.map((ranking, index) => (
                                                        <tr key={index}><td>{ranking.tipo.replace(/_/g, ' ')}</td><td>{ranking.juego?.nombre || 'Overall'}</td><td>
                                                            ...
                                                            <Badge
                                                                bg={
                                                                    ...
                                                                    ranking.posicion === 3 ? 'danger' : 'info'
                                                                }
                                                            >
                                                                #{ranking.posicion}
                                                            </Badge>
                                ...
                                <Tab eventKey="globalRankings" title={<><FaChartLine className="me-1" /> Global Rankings</>}>
                                    {/* ... (Global Rankings content - needs implementation) ... */}
                                     <div className="p-3">
