...
const ruletaService = {
    /**
     * Calls the backend to place a bet and process it. The winning number is generated by the backend.
     * Assumes backend endpoint is POST /api/juegos/ruleta/jugar
     *
     * @param {object} betData - { usuarioId, cantidad, tipoApuesta, valorApostado }
     * @returns {Promise<object>} - Promise resolving to { winningNumber, resolvedBet (processed bet details) }
     */
    jugar: (betData) => {
        ...
        return axios.post(`${API_URL}/jugar`, params, { // Assumes axios instance has baseURL '/api' or similar
            ...
            // { winningNumber: (the randomly generated number), resolvedBet: { ...bet details... } }
            if (response.data && typeof response.data === 'object' && response.data.resolvedBet) {
                ...
                return {
                    winningNumber: response.data.winningNumber, // Use the number generated by the backend
                ...
                const sanitizedFallback = sanitizeApuesta(fallbackBet);
                 ...
                 // Return a default winning number and the potentially incomplete/sanitized bet data
                 // This indicates a potential issue with the backend response structure.
                 return {
                     winningNumber: 0, // Default to 0 if no winning number was returned
